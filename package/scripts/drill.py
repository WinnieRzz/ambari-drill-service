...
class Master(Script):
  def install(self, env):
    ...
    if not os.path.exists(params.drill_temp_file):
      Execute('wget http://apache.mirrors.hoobly.com/drill/drill-1.10.0/apache-drill-1.10.0.tar.gz -O ' + params.drill_temp_file + ' -a ' + params.drill_log_file, user=params.drill_user)
  ...
  def stop(self, env):
    import params
    self.configure(env)
    Execute(params.drill_install_dir + '/apache-drill-1.10.0/bin/drillbit.sh stop', user=params.drill_user)

  def start(self, env):
    import params
    self.configure(env)
    Execute(params.drill_install_dir + '/apache-drill-1.10.0/bin/drillbit.sh start', user=params.drill_user)
  ...
  def configure(self, env, isInstall=False):
    ...
    File(params.drill_install_dir + '/apache-drill-1.10.0/conf/drill-override.conf', content=drill_override_content, owner=params.drill_user, group=params.drill_group)
    File(params.drill_install_dir + '/apache-drill-1.10.0/conf/drill-env.sh', content=drill_env_content, owner=params.drill_user, group=params.drill_group)
    XmlConfig("hdfs-site.xml", 
              conf_dir=params.drill_install_dir + '/apache-drill-1.10.0/conf',
